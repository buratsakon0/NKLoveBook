    ...
    <div class="bg-white shadow-md rounded-lg border border-gray-200">
      ...
      <div class="flex justify-between items-center p-6 border-b border-gray-200">
        <div class="flex items-center space-x-4">
          <img src="{{ $item->image }}" alt="{{ $item->name }}" class="w-24 h-auto rounded shadow-sm">
          <div>
            <p class="font-semibold text-indigo-900">{{ $item->name }}</p>
            <p class="text-sm text-gray-500 mt-1">{{ $item->author ?? 'Unknown Author' }}</p>
            <p class="text-gray-600 mt-1">฿{{ number_format($item->price, 2) }}</p>
          </div>
        </div>

        <div class="flex items-center space-x-4">
          <form action="{{ route('wishlist.remove', $item->id) }}" method="POST" class="flex items-center">
            @csrf
            @method('DELETE')
